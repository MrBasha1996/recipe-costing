...
export default function BatchesClient({ brand }: { brand: BrandId }) {
  ...
  async function handleExport() {
    const supabase = createClient()
    try {
      await exportBatches(brand, supabase)
    } catch (e: any) {
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {canE && (
          <div className="flex gap-2 flex-wrap justify-end">
            ...
            <Link
              href={`/${brand}/batches/import`}
              className="text-sm px-3 py-1.5 border border-orange-300 bg-orange-50 rounded-lg hover:bg-orange-100 text-orange-700 transition-colors"
            >
              ⬆ استيراد وصفات
            </Link>
            <button
