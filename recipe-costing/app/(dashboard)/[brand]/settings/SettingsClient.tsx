...
export default function SettingsClient({ initialCommission, initialFcLow, initialFcHigh }: Props) {
  ...
  const [commissionPct, setCommissionPct] = useState<string>(String(initialCommission))
  const [savingCommission, setSavingCommission] = useState(false)
  const [commissionMsg, setCommissionMsg] = useState<{ ok: boolean; text: string } | null>(null)

  const [fcLow, setFcLow]   = useState<string>(String(initialFcLow))
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-3">
          <div className={`flex items-center justify-between rounded-xl px-4 py-3 border ${isCurrentClosed ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'}`}>
            <div className="flex items-center gap-2">
              <span>{isCurrentClosed ? '🔒' : '🟢'}</span>
              <div>
                <div className="text-sm font-medium text-gray-800">الفترة الحالية — {formatYearMonth(currentYM)}</div>
            ...
            <button onClick={() => setShowPeriodManager(true)}
              ...
              <div className="space-y-1">
                {closedPeriods.slice(0, 5).map(p => (
                  <div key={p.id} className="flex items-center justify-between text-xs text-gray-600 bg-gray-50 rounded-lg px-3 py-2">
                    <span className="flex items-center gap-2"><span>🔒</span>{formatYearMonth(p.year_month)}</span>
                    <span className="text-gray-400 font-mono">{new Date(p.closed_at).toLocaleDateString('ar-SA')}</span>
                  </div>
                ))}
                {closedPeriods.length > 5 && (
                  <button onClick={() => setShowPeriodManager(true)} className="text-xs text-blue-500 hover:text-blue-700 px-3">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          {commissionMsg && <p className={`text-xs ${commissionMsg.ok ? 'text-green-600' : 'text-red-600'}`}>{commissionMsg.text}</p>}
        ...
        <div className="px-6 py-4 space-y-3">
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <input type="number" min="0" max="100" step="0.5" value={fcLow} onChange={e => setFcLow(e.target.value)}
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="number" min="0" max="100" step="0.5" value={fcHigh} onChange={e => setFcHigh(e.target.value)}
                className="w-20 border border-gray-300 rounded-lg px-3 py-1.5 text-sm text-left font-mono focus:outline-none focus:border-blue-500" />
              <span className="text-sm text-gray-500">%</span>
            </div>
          ...
          {fcMsg && <p className={`text-xs ${fcMsg.ok ? 'text-green-600' : 'text-red-600'}`}>{fcMsg.text}</p>}
