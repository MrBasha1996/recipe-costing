-- =====================================================
-- Migration 027: Atomic WAC (Weighted Average Cost) RPC
-- =====================================================
-- المشكلة: حساب WAC يقرأ stock_qty و cost ثم يكتب — إذا جرى استيرادان
-- متزامنان يقرآن نفس الرصيد → ينتج WAC خاطئ.
-- الحل: SELECT FOR UPDATE يقفل الصف أثناء الحساب.
...
BEGIN
  ...
  LOOP
    ...
    INTO v_current_qty, v_current_cost  -- reuse v_current_cost as min_qty placeholder
    FROM stock_items
